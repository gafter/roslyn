...
8");
        ...
        [Fact]
        public void NullableArrayDeclarationPattern_Bad_01()
        {
            var source =
@"#nullable enable
public class A
{
    ...
    {
        if (o is A[]?[,] b3) { }          // error 3 (nullable type in decl pattern)
        if (o is A[,]?[] b4 && c) { }     // error 4 (nullable type in decl pattern)
        if (o is A[,]?[]?[] b5 && c) { }  // error 5 (nullable type in decl pattern)
    }
}
";
            var compilation = CreatePatternCompilation(source, options: TestOptions.DebugDll);
            compilation.VerifyDiagnostics(
                // (6,18): error CS8116: A pattern type may not be nullable; use the underlying type 'A[][*,*]' instead.
                //         if (o is A[]?[,] b3) { }          // error 3 (nullable type in decl pattern)
                Diagnostic(ErrorCode.ERR_PatternNullableType, "A[]?[,]").WithArguments("A[][*,*]").WithLocation(6, 18),
                // (7,18): error CS8116: A pattern type may not be nullable; use the underlying type 'A[*,*][]' instead.
                //         if (o is A[,]?[] b4 && c) { }     // error 4 (nullable type in decl pattern)
                Diagnostic(ErrorCode.ERR_PatternNullableType, "A[,]?[]").WithArguments("A[*,*][]").WithLocation(7, 18),
                // (8,18): error CS8116: A pattern type may not be nullable; use the underlying type 'A[*,*][]?[]' instead.
                //         if (o is A[,]?[]?[] b5 && c) { }  // error 5 (nullable type in decl pattern)
...
";
        ...
        [Fact]
        public void NullableArrayDeclarationPattern_Good()
        {
            var source =
@"#nullable enable
public class A
