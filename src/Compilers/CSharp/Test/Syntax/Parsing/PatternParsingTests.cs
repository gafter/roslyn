...
@"e switch {
    ...
    {X: C(1)} => 8
        ...
        [Fact]
        public void IsNullableArray04()
        {
            // OK, this means `(o is A[][]? b) && c` because `A[][]?` is not a nullable type
            UsingExpression("o is A[][] ? b && c");
            N(SyntaxKind.LogicalAndExpression);
            {
                N(SyntaxKind.IsPatternExpression);
        ...
        [Fact]
        public void IsNullableArray05()
        {
            // Semantic error, because the declaration pattern's type is nullable
            UsingExpression("o is A[]?[] b && c");
            N(SyntaxKind.LogicalAndExpression);
            {
                N(SyntaxKind.IsPatternExpression);
        ...
        [Fact]
        public void IsNullableArray07()
        {
            // OK: nullable types are permitted in a type test
            UsingExpression("o is A[]? && c");
            N(SyntaxKind.LogicalAndExpression);
            {
                N(SyntaxKind.IsExpression);
                {
